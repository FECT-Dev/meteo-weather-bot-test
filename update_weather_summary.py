...
for date_folder in sorted(os.listdir(reports_folder)):
    ...
    for file in os.listdir(folder_path):
        ...
        try:
            ...
            for line in text.splitlines():
                # Remove extra symbols and double spaces
                ...
                # Try best guess for station
                station_guess = line
                for num in numbers: station_guess = station_guess.replace(num, "")
                station_guess = station_guess.strip()

                match = get_close_matches(station_guess.title(), known_stations, n=1, cutoff=0.7)
                if not match:
                    unmatched.append(f"{station_guess} | {numbers} | {line}")
                    continue
