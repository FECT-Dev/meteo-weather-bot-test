...
# Updated regex with better flexibility and capture groups
line_pattern = re.compile(
    r"^([A-Za-z][A-Za-z \-]{2,})\s+(TR|\d{1,2}(?:\.\d)?)\s+(TR|\d{1,2}(?:\.\d)?)\s+(TR|\d{1,3}(?:\.\d)?)$"
)
...
for date_folder in sorted(os.listdir(reports_folder)):
    ...
    for file in os.listdir(folder_path):
        ...
        try:
            ...
            # 🔍 Debug: print the first 20 lines of OCR text
            ...
            if found_any_station:
                ...
                print(f"❌ No station data matched in {file}. Check OCR output.")
